...
import 'package:dezon/views/profile/modifyPassword.dart';
...
class UserProfile extends StatefulWidget {
  final int id;
  final bool canEdit;
  UserProfile({this.canEdit = false, @required this.id});
  @override
...
class _UserProfileState extends State<UserProfile> {
  ...
  Future<Map> futureProfileData;

  Future<Map> fetchProjectData() async {
    const String whatWeGettin = "Profile's Data";
    ...
    if (response.statusCode.toString().startsWith('20')) {
      ...
      return Map.from(jsonDecode(response.body))["body_response"];
    } else {
      printGetFailed(whatWeGettin);
      throw Exception("Failed to load Profile's Data");
    }
  }

  @override
  void initState() {
    futureProfileData = fetchProjectData();
    super.initState();
  }
      ...
      body: FutureBuilder<Map>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.done &&
            ...
            infos = ValueNotifier(
              {
                'freelance-photo-profile': respBody['freelance-photo-profile'],
                'user-login': Map.from(
